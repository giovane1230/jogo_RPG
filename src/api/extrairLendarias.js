...
function extrairLendarias(texto) {
  ...
  for (let i = 0; i < linhas.length; i++) {
    if (linhas[i].startsWith('Reação de')) {
      const monstro = linhas[i].replace('Reação de ', '').trim();
...
Reação de Stone Giant

Rock Catching

If a rock or similar object is hurled at the giant, the giant can, with a successful DC 10 Dexterity saving throw, catch the missile and take no bludgeoning damage from it.`; // seu texto aqui
...
fs.writeFile('reacao.json', JSON.stringify(resultado, null, 2), (err) => {
  if (err) {
